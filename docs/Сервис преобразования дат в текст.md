...
- Потребуется отдельная реализация для методов ```getDate()``` и ```getCapitalizedDate()```, про будущии методы тоже не
  забываем;
...
- **Наследование** - когда, уверены, что дальше наследования не потребуется и наш функционал всегда будет в одном
  классе,
  но нужны разные форматы;
- **Отдельный метод** - когда, уверены, что класс будет расширяться часто, но базовые методы приведения к формату всегда
  будут одни.

---

### Реализация через методный вариант

#### Реализовать класс для вывода даты в текстовом формате

Создадим класс ```UserFriendlyDateTimeMethod``` и добавим метод для
...
[Пример реализации](src/FriendlyDate/Method/UserFriendlyDateTimeMethod.php)

@todo описать возможные недостатки

#### Реализовать вариант написания текста как строчными, так и с заглавной

Рассмотрим варианты:

- [Параметр в методе](#method-capitalized-paratemer);
- [Отдельный метод](#method-capitalized-self-method);
- [Наследование](#method-inherit);
- [Декоратор](#method-decorator).
...
Данный вариант решения копирует [аналогичное](#method-capitalized-parameter). 

##### Отдельный метод

<a name="method-capitalized-self-method"></a>

Данный вариант решения копирует [аналогичное](#method-capitalized-self-method).

##### Наследование

<a name="method-inherit"></a>

Данный вариант решения копирует [аналогичное](#method-capitalized-inherit).

##### Декоратор

<a name="method-decorator"></a>

Данный вариант решения копирует [аналогичное](#method-capitalized-decorator).
...
[Пример реализации](src/FriendlyDate/Method/UserFriendlyDateTimeMethodExtend.php)

Разница между реализации из [прошлой части](#extended-by-inherit), не отличается от текущей.

##### Расширение через декоратор

[Пример реализации](src/FriendlyDate/Constructor/UserFriendlyDateTimeDecorator.php)

Относительно [прошлого решения](#extended-by-decorator), мы получаем совсем другой результат.
...
Данный вариант решения копирует [аналогичный](#self-date-time-format-parameter).

##### Отдельный метод

Данный вариант решения копирует [аналогичный](#self-date-time-format-self-method).

##### Расширение через наследование

Данный вариант решения копирует [аналогичный](#self-date-time-format-inherit).
